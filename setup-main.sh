...
# --- Definisi Variabel ---
REPO="https://raw.githubusercontent.com/FzTunneling/scriptprem/ABSTRAK/"
start=$(date +%s)
secs_to_human() {
...
function udp_mini(){
    ...
    wget https://raw.githubusercontent.com/FzTunneling/scriptprem/ABSTRAK/files/limit.sh && chmod +x limit.sh && ./limit.sh
    print_ok "Mengunduh limit-ip..."
    wget -q -O /usr/bin/limit-ip "https://raw.githubusercontent.com/FzTunneling/scriptprem/ABSTRAK/files/limit-ip"
...
EOF
    ...
    wget -q -O /usr/local/kyt/udp-mini "https://raw.githubusercontent.com/FzTunneling/scriptprem/ABSTRAK/files/udp-mini"
    chmod +x /usr/local/kyt/udp-mini
    print_ok "Mengunduh dan mengelola layanan udp-mini..."
    for i in {1..3}; do
        wget -q -O /etc/systemd/system/udp-mini-${i}.service "https://raw.githubusercontent.com/FzTunneling/scriptprem/ABSTRAK/files/udp-mini-${i}.service"
    done
    for i in {1..3}; do
        systemctl daemon-reload
...
function install_openvpn() {
    ...
    if wget https://raw.githubusercontent.com/FzTunneling/scriptprem/ABSTRAK/files/openvpn -O /root/openvpn_setup.sh; then
